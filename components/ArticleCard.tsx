...
export default function ArticleCard({ id, title, date, rating, text, previewUrl, articleUrl, onDelete, onUpdate }: ArticleCardProps) {
    ...
    <article className="flex gap-4 relative">
      {previewUrl && (
        <a href={articleUrl} target="_blank" rel="noopener noreferrer">
          <img src={previewUrl} alt={`${title} photo big card`} className="hidden w-[200px] h-auto sm:block md:hidden lg:w-[150px] lg:block xl:w-[200px]" width={200} height={200} loading="lazy" />
        </a>
      )}
      <div className="w-[100%]">
        {onUpdate && !isEditing && (
        ...
        <div className="flex">
          {previewUrl && (
            <a href={articleUrl} target="_blank" rel="noopener noreferrer">
              <img src={previewUrl} alt={`${title} photo small card`} className="w-[100px] h-auto mr-4 sm:hidden md:block lg:hidden" width={100} height={100} loading="lazy" />
            </a>
          )}
          <div>
            <a href={articleUrl} target="_blank" rel="noopener noreferrer" className="hover:underline">{title}</a>
            <p>Date: {date}</p>
            {isEditing ? (
              <div>
